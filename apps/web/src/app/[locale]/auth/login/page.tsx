...
export default function LoginPage() {
  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!email || !password) {
      setError(copy.auth.enterCredentials);
      return;
    }
  ...
  const handleExternalLogin = (provider: 'WeChat' | 'Alipay') => {
    setError(`${provider} ${copy.auth.externalNotConfigured}`);
  };
    ...
    <div className="min-h-screen bg-ink-950 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-100">{copy.auth.welcome}</h1>
          <p className="text-slate-400 mt-2">{copy.auth.loginSubtitle}</p>
        </div>
        ...
        <div className="bg-ink-900 rounded-2xl p-6 border border-ink-800">
            ...
            <div className="text-center space-y-4">
              <div className="w-48 h-48 mx-auto bg-white rounded-2xl p-4">
                <div className="w-full h-full bg-slate-100 rounded-xl flex items-center justify-center">
                  <p className="text-slate-400 text-sm">{copy.auth.wechat} {copy.auth.qrCode}</p>
                </div>
              </div>
              ...
              >
                {copy.auth.externalLogin} {copy.auth.wechat}
              </button>
            </div>
          )}
          ...
          {mode === 'alipay' && (
            <div className="text-center space-y-4">
              <div className="w-48 h-48 mx-auto bg-white rounded-2xl p-4">
                <div className="w-full h-full bg-slate-100 rounded-xl flex items-center justify-center">
                  <p className="text-slate-400 text-sm">{copy.auth.alipay} {copy.auth.qrCode}</p>
                </div>
              </div>
              {error && <p className="text-sm text-danger-500">{error}</p>}
              <button
              ...
              >
                {copy.auth.externalLogin} {copy.auth.alipay}
              </button>
            </div>
          )}
        </div>
